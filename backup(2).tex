...
The publication trend reported in (Figure~7.1) confirms a consistent increase in research output on household carbon footprint estimation using input-output models since 2010. Annual publications reached a peak of 27 in 2020 and remained elevated through 2024, reflecting sustained methodological interest and broader policy relevance consistent with findings reported by Sheng et al. (2024).

\begin{figure}[h]
\centering
\includegraphics[width=0.7\textwidth]{Publications.png}
\caption*{\textbf{Figure 7.1:} Annual publications on household carbon footprint using input-output models (2008--2025)}\label{fig:Publications}
\end{figure}

\subsubsection{Lead Author Contributions}

The analysis shows that lead authorship is concentrated among a small number of prolific researchers, including Long Y.~(9 publications) and Shigetomi Y.~(5 publications). The top contributors are affiliated with institutions in China, Japan, Finland, Norway, and the UK, indicating geographically diverse but institutionally clustered leadership in the field (Table~7.1).

\begin{table}[h]
\centering
\caption*{\textbf{Table 7.1:} Lead authors and affiliations in Input-Output based Household Footprint Research.}\label{tab:lead_authors_affiliations}
\begin{tabular}{@{}lrl@{}}
\hline 
...
The co-authorship profile indicates that the majority of studies are produced by multi-author teams, which is consistent with the collaborative nature of quantitative input-output research and the integration of environmental extensions with multi-regional datasets (Figure~7.2).

\begin{figure}[h]
\centering
\includegraphics[width=0.7\textwidth]{Coauthors.png}
\caption*{\textbf{Figure 7.2: }Distribution of co-authorship in household carbon footprint IO publications}\label{fig:coauthorship}
\end{figure}

\subsubsection{Top Source Journals}

The leading publication outlets include the \textit{Journal of Cleaner Production}, \textit{Ecological Economics}, and \textit{Environmental Research Letters}. Most high-volume journals are published in the Netherlands, the UK, or the USA, reflecting the prominent role of major international publishers and the concentration of environmental economics and industrial ecology journals within the European and North American research landscape (Table~7.2).

\begin{table}[h]
\centering
\caption*{\textbf{Table 7.2: }Top Source Journals, Number of Publications, and Publishing Countries}\label{tab:journal_countries}
\begin{tabular}{lll}
\hline
